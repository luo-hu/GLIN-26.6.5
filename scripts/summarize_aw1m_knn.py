...
def main():
    paths = [
        path for path in sorted(Path("results").glob("aw1m_knn_*.csv"))
        if selectivity_from_name(path) != "unknown"
    ]
    if not paths:
        raise SystemExit("No raw aw1m_knn benchmark CSV files found in results/")

    summaries = [summarize_file(path) for path in paths]
    summaries.sort(key=sort_key)

    output_path = Path("results/aw1m_knn_summary.csv")
    fields = [
        "selectivity",
        "dataset",
        "index",

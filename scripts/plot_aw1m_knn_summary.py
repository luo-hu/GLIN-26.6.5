...
def plot_metric_by_relationship(rows, output_dir, metric, ylabel, title, filename, dpi):
    ...
    for ax, relationship in zip(axes, RELATIONSHIP_ORDER):
        ...
        setup_axis(ax, relationship.capitalize(), ylabel)
...
def plot_probe_refine_breakdown(rows, output_dir, dpi):
    selectivities = ordered_selectivities(rows)
    figure_paths = []
    ...
    for relationship in RELATIONSHIP_ORDER:
        ...
        path = output_dir / f"aw1m_knn_{relationship}_probe_refine.png"
        fig.savefig(path, dpi=dpi)
        plt.close(fig)
...
def plot_build_time(rows, output_dir, dpi):
    ...
    path = output_dir / "aw1m_knn_build_time.png"
    fig.savefig(path, dpi=dpi)
    plt.close(fig)
    return path
...
def main():
    ...
    figure_paths = [
        plot_metric_by_relationship(
            ...
            title="AREAWATER 1M KNN Query Time",
            filename="aw1m_knn_query_time.png",
            dpi=args.dpi,
        ),
        plot_metric_by_relationship(
            rows,
            ...
            title="AREAWATER 1M Candidate Count",
            filename="aw1m_knn_candidates.png",
            dpi=args.dpi,
        ),
        plot_metric_by_relationship(
            rows,
            ...
            title="AREAWATER 1M Answer Count",
            filename="aw1m_knn_answers.png",
            dpi=args.dpi,
        ),
        plot_build_time(rows, output_dir, args.dpi),
    ]
    figure_paths.extend(plot_probe_refine_breakdown(rows, output_dir, args.dpi))

// Boost R-tree baseline：读取同一份 WKT 数据和 query 文件，输出与 GLIN benchmark 相同格式的 CSV。
//
// 实验流程：
// 1. 读取 AREAWATER.csv，解析 WKT 为 GEOS Geometry。
// 2. 用每个 geometry 的 MBR 建 Boost R-tree。
// 3. 读取固定 query_file，query_file 由 generate_wkt_queries 生成。
// 4. R-tree 用 MBR intersects 找候选；GEOS 再做 contains/intersects 精确过滤。
//
// 注意：这是第一版 baseline，目的是先让 GLIN 有可比较对象。论文完整复现还需要
// JTS KNN selectivity query、更多数据集、QuadTree baseline 和重复实验。
...
bool exact_match(const std::string& relationship, Geometry* query, Geometry* candidate) {
...
RTree build_rtree(const std::vector<GeometryPtr>& geometries) {
...
void write_csv(const std::string& path, const Options& options,
  ...
  for (const auto& result : results) {
    output << "AREAWATER,Boost_Rtree," << options.relationship << ","
           ...
           << (result.probe_ns + result.refine_ns) << "," << result.candidates
           << "," << result.answers << ",0,0\n";
  }
}
...
int main(int argc, char* argv[]) {
  try {
    ...
    RTree rtree = build_rtree(geometries);
    ...
    for (const auto& query : queries) {
      ...
      Box query_box = box_from_query(query);

      auto probe_start = std::chrono::high_resolution_clock::now();
      rtree.query(bgi::intersects(query_box), std::back_inserter(candidates));

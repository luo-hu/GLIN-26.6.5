// DELI-Dynamic-Single prototype for exact Intersects over WKT geometries.
//
// This benchmark isolates the dynamic extent-maintenance mechanism before
// wiring it into GLIN/ALEX leaves. Records are append-only and addressed by
// stable record ids; each block stores a sorted vector of record ids and
// maintains one-sided conservative summaries.
...
struct Options {
  ...
  double stale_threshold_fraction = 0.1;
...
void print_usage(const char* program) {
  std::cerr
      ...
      << "  --stale_threshold_fraction F     Dead-count rebuild threshold as block fraction (default: 0.1)\n"
...
int main(int argc, char* argv[]) {
  try {
    ...
    std::vector<CheckpointSummary> summaries;
    summaries.reserve(3);
    summaries.push_back(run_checkpoint(options, geometries, index, queries,
                                       "after_bulkload", 0, initial_count, 0, 0,
                                       0, 0));
    print_checkpoint_summary(summaries.back());
    ...
    summaries.push_back(run_checkpoint(options, geometries, index, queries,
                                       "after_insert", 1, initial_count,
                                       insert_count, 0, insert_ns, 0));
    print_checkpoint_summary(summaries.back());

    std::vector<ObjectId> delete_ids = index.live_object_ids();
    ...
    summaries.push_back(run_checkpoint(options, geometries, index, queries,
                                       "after_delete", 2, initial_count,
                                       insert_count, delete_count, insert_ns,
                                       delete_ns));
    print_checkpoint_summary(summaries.back());

    write_csv(options.output_csv, options, stats, summaries);
